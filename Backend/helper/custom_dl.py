...
class ByteStreamer:
    ...
    async def prefetch_stream(
        ...
        registry_entry = {
            ...
            "prefetch": prefetch,
            "meta": meta or {},
            # Kullanıcı bazlı hız limiti — stream_routes tarafından dinamik olarak
            # güncellenir; 0.0 = sınırsız.  Throttle döngüsü her chunk'ta buradan okur.
        ...
        async def consumer_generator():
            ...
            # Hız limiti: ACTIVE_STREAMS'den dinamik okunur (stream_routes kullanıcının
            # toplam aktif stream sayısına göre her stream'in payını günceller).
            # _rate_bytes_per_sec her chunk'ta yeniden hesaplanır; 0 = sınırsız.
            _throttle_start: float = time.monotonic()
            _throttle_sent: int = 0  # bu pencerede gönderilen byte
            ...
            try:
                while True:
                    ...
                    if part_count == 1:
                        out = chunk[first_part_cut:last_part_cut]
                    elif current_part_idx == 1:
                        out = chunk[first_part_cut:]
                    elif current_part_idx == part_count:
                        out = chunk[:last_part_cut]
                    else:
                        out = chunk

                    # ── Hız limiti throttle (dinamik — kullanıcı bazlı) ──────────
                    _current_limit = ACTIVE_STREAMS.get(stream_id, {}).get("rate_limit_mbps", 0.0)
                    _rate_bytes_per_sec = (_current_limit * 1024 * 1024 / 8) if _current_limit > 0 else 0.0
                    if _rate_bytes_per_sec > 0:
                        _throttle_sent += len(out)
                        elapsed_window = time.monotonic() - _throttle_start
                        expected_time  = _throttle_sent / _rate_bytes_per_sec
                    ...
                    yield out
                    _out_post_ts = time.time()

                    # ── Outbound (sunucudan dışarı çıkan) anlık hız ──────
                    _out_elapsed = max(_out_post_ts - _out_pre_ts, 1e-6)
                    _out_bps = len(out) / _out_elapsed
                    _entry = ACTIVE_STREAMS.get(stream_id)
                    if _entry is not None:
                        _prev_out = _entry.get("out_recent", [])
                        _prev_out.append((_out_bps, _out_elapsed))
                        if len(_prev_out) > 4:
                            _prev_out = _prev_out[-4:]
                        _entry["out_recent"] = _prev_out
                        _w_bytes = sum(b * t for b, t in _prev_out)
                        _w_time  = sum(t for _, t in _prev_out)
                        _entry["out_instant_bps"] = int(_w_bytes / max(_w_time, 1e-6))
                    # ────────────────────────────────────────────────────
