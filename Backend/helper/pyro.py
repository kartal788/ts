...
def clean_filename(filename):
    ...
    # Strip embedded URLs (e.g. https://www.themoviedb.org/tv/253645) before any parsing
    filename = re.sub(r'https?://\S+', '', filename).strip()

    # Unicode matematik harfleriyle yazılmış "𝐾𝑜𝑟𝑃𝑖𝑟𝑎𝑐𝑦" gibi watermark'ları temizle
    import unicodedata
    filename = unicodedata.normalize("NFKD", filename)
    # "... KorPiracy.mkv" → sadece asıl dosya adını bırak
    filename = re.sub(r'\s*KorPiracy\.mkv\b', '', filename, flags=re.IGNORECASE).strip()
    # Klasör yolu içeriyorsa (path/file.mkv) → sadece son kısmı al
    if '/' in filename:
...
BotCommand("ekle", "📥 Google drive klasöründeki dizi/filmleri stremioya ekler/siler."),
